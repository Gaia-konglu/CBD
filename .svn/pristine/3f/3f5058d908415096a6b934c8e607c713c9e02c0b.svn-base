...
<input id="companyName" type="hidden" value="${sessionScope.name}"/>
     <script>
     	function send(ppage){
     		...
     		$.ajax({
     			url:"/CBDParkingManagementSystem/showBookInfor.do",
				type:"post",
				data:{
					"page":ppage
				},
					...
					for(var i=0;i<books.length;i++){
						...
						content += "<tr>"
								+"<td><input type='checkbox' name='IDCheck' value='14458579642011' class='acb' /></td>"
				     			+"<td>"+book.part.addr+"</td>"
				     			+"<td>"+book.part.carNum+"</td>"
				     			+"<td>"+book.rent.startTime+"</td>"
				     			+"<td>"+book.rent.endTime+"</td>"
				     			+"<td>"+book.tenant.name+"</td>"
				     			+"<td>"+book.tenant.job+"</td>"
				     			+"<td>"+book.tenant.phone+"</td>"
				     			+"<td>"+book.tenant.credit+"</td>"
				     			+"<td>"+book.message+"</td>"
				    			+"<td><span class='point' onclick='affirm("+book.id+","+book.rent.id+")'>确认出租</span></td>"
		     					+"</tr>";
					}
    	...
    	var jsonobj = {"bid":b_id,"rid":r_id};
    		$.ajax({
    			url:"/CBDParkingManagementSystem/updateStatus.do",
    			type:"post",
    			data:jsonobj,
