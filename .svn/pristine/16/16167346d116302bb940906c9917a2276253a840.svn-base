...
<shiro:hasPermission name="contractMP">
 ...
 <shiro:hasPermission name="carportMP">
		...
		<form action="addnewthirdparkcarport.do" method="post"
			name="channelform" onsubmit="return validate_channel_info(this);">
			<div class="form-group" style="clear:both">
				...
				<div class="col-sm-8">
					<input readonly="readonly" type="text" class="form-control1"
						...
						value="<%=session.getAttribute("thirdcontractnum")%>">
			...
			<div class="form-group" style="clear:both">
				...
				<div class="col-sm-8">
					<input type="text" class="form-control1" id="company"
						name="company" placeholder="如：成都蜗牛学院" onfocus="YHMonfocu()"
						onblur="YHMonblus()"> <font color=red><span
						id="YHMerror"></font>
			...
			<div class="form-group" style="clear:both">
				<label for="focusedinput" class="col-sm-2 control-label">地址信息:</label>
				<div class="col-sm-8">
					<input type="text" class="form-control1" id="address"
						name="address" placeholder="如：成都市高新区孵化园区5号楼" onfocus="addronfocu()"
						onblur="addronblus()"> <font color=red><span
						id="addrerror"></font>
			...
			<div class="form-group" style="clear:both">
				...
				<div class="col-sm-8">
					<input type="text" class="form-control1" id="contacts"
						name="contacts" placeholder="如：张三" onfocus="contactsonfocu()"
						onblur="contactsonblus()"> <font color=red><span
						id="contaerror"></font>
				</div>
				...
				<div class="form-group" style="clear:both">
					...
					<div class="col-sm-8">
						<input type="text" class="form-control1 input-sm" id="phone"
							name="phone" placeholder="如：13909987898" onfocus="phoneonfocu()"
							onblur="phoneonblus()"> <font color=red><span
							id="phoneerror"></font>
					</div>
				</div>
				...
				<div class="form-group" style="clear:both">
					...
					<div class="col-sm-8">
						<input readOnly="true" style="height:25px;width:300px"
							class="Wdate" type="text" id="startDate" name="startDate"
							onclick="WdatePicker({maxDate:'#F{$dp.$D(\'endDate\')}'});"
							onfocus="startDateonfocu()" onblur="startDateonblus()" /> <font
							color=red><span id="startDateerror"></font>
					</div>
				</div>
				...
				<div class="form-group" style="clear:both">
					...
					<div class="col-sm-8">
						<input readOnly="true" style="height:25px;width:300px"
							class="Wdate" type="text" id="endDate" name="endDate"
							onclick="WdatePicker({minDate:'#F{$dp.$D(\'startDate\')}'});"
							onfocus="endDateonfocu()" onblur="endDateonblus()" /> <font
							color=red><span id="endDateerror"></font>
					</div>
				</div>
    ...
    function validate_channel_info(channelform)
					...
					function YHMonblus() {
						...
						} else if (company.value.length<1 ||company.value.length>18) {
							console.log(company.value);
							document.getElementById('YHMerror').innerText = "格式错误,长度应为1-18个字符";
						} else {
					...
					function addronblus() {
						...
						var re = /^[a-zA-Z_]{6,18}$/;
						if (address.value == "") {
							document.getElementById('addrerror').innerText = "请输入地址信息";
						} else if (address.value.length<5 ||address.value.length>18) {
							console.log(address.value);
							document.getElementById('addrerror').innerText = "格式错误,长度应为5-18个字符";
						} else {
