		...
		<div class="ui_content">
				<div class="ui_text_indent">
					<div id="box_border">
						...
						<div id="box_center">
							车位地址
							<input placeholder="请输入" type="text" name="fangyuanEntity.fyXqCode" id="fyXq" class="ui_select01" onchange="getFyDhListByFyXqCode();"/>
							车位编号
							<input placeholder="请输入" type="text" name="fangyuanEntity.fyDhCode" id="fyDh" class="ui_select01"/>
							最长可租截至日期
							<input type="date" name="fangyuanEntity.fyHxCode" id="fyHx" class="ui_select01"/>
				...
				<div class="ui_tb_h30">
					...
					<div class="ui_frt">
						<!--    如果是第一页，则只显示下一页、尾页 -->
							...
							<input onclick="companyNotRentedList(1)" type="button" value="首页" class="ui_input_btn01" />
...
<input id="companyName" type="hidden" value="${sessionScope.name}"/>
     <script>
     	function companyNotRentedList(ppage){
     		/* var jsonobj = {"name":"${sessionScope.name}"}; */
     		$.ajax({
     			url:"/CBDParkingManagementSystem/notRent.do",
				type:"post",
				data:{
					page:ppage,
					addr:$("#addr").val(),
		...
		//	alert("end");
			var total=$("#totalpage").html();
			var totalPage=parseInt(total);
			companyNotRentedList(totalPage);
		}
		function jump(){
			//先获取input框用户输入的值
			...
			if(jumpValue >= 1 && jumpValue <= total){
				companyNotRentedList(jumpValue);
			}
		}
		/** 模糊查询企业已租赁车位 **/
		function search(){
     ...
     <!-- <div class="bottom_buttons">
    		<button name="first" onclick="companyNotRentedList(1)">首页</button>
    		...
    		进行跳转
    		跳转到<input id="jump" type="text"/>页
    		<button name="jump" onclick="jump()">跳转</button>
		</div> -->
</body>
</html>